...
function array(arr){
  // checking to see if string is too small to remove first and last char.
  if(arr.length <= 1){  
    return null
  }
  
  // convert to string to array and remove first and last char
  // one more check to make sure its not empty and returning array as a string
let string = arr.split(',')
string.shift(),string.pop()
  if(string == ''){
    return null
  }
  else{
